...
public class NewEDNAPluginWizard extends Wizard implements INewWizard, ModifyListener {
	...
	private void initialiseModel() {
		getEdnaHome();
		//TODO these should all be in properties.
	...
	protected void copyPlugin(IProgressMonitor monitor) throws CoreException {		
		...
		EDNAPluginGeneratorModel emulatedModel = new EDNAPluginGeneratorModel();
		emulatedModel.populateFromFile(input);
		IFile copy = WizardHelpers.generateFileName(input, (IFolder)input.getParent(), emulatedModel.getName(), model.getName());
		HashMap<String, String> substitutionMap = new HashMap<String, String>();
		substitutionMap.put(emulatedModel.getName(), model.getName());
		substitutionMap.put(emulatedModel.getAuthor(), model.getAuthor());
		substitutionMap.put(emulatedModel.getCopyright(), model.getCopyright());		
		WizardHelpers.copyAndUpdateFile(input, copy, substitutionMap, monitor);

		// now copy any associated tests
		IFolder testData = input.getParent().getFolder(new Path("../tests/data"));
		IFolder testSuite = input.getParent().getFolder(new Path("../tests/testsuite"));

		String searchString = input.getName().replace(".py", "");
		// look in all the test files for this name, and if it is there, then prepare to copy them
		ArrayList<IFile> copyList = new ArrayList<IFile>();

		IResource[] members = testSuite.members();
		for (int i = 0; i < members.length; i++) {
			if(members[i] instanceof IFile) {
				if(WizardHelpers.containsString((IFile) members[i], searchString)) {
					copyList.add((IFile) members[i]);
					List<String> possibleReferences = WizardHelpers.allReferencedFilesinFile((IFile) members[i], ".xml");
					for (String reference : possibleReferences) {
						IFile ref = testData.getFile(reference);
						if(ref.exists()) {
							copyList.add(ref);
						}
					}
		...
		// now copy all these referenced items, remembering to include odd file names
		ArrayList<IFile> copyToList = new ArrayList<IFile>();
		for (IFile file : copyList) {
			IFile filecopy = WizardHelpers.generateFileName(file, (IFolder)file.getParent(), emulatedModel.getName(), model.getName());
			copyToList.add(filecopy);
			substitutionMap.put(file.getName(), filecopy.getName());
		}

		// Now do the actual copy
		for (int i = 0; i < copyList.size(); i++) {
			WizardHelpers.copyAndUpdateFile(copyList.get(i), copyToList.get(i), substitutionMap, monitor);
		}
	}
				...
				InterruptedException {
					...
					try {
						if (model.getPluginCopyMode().equals(EDNAPluginGeneratorModel.COPY)) {
							// deal with making the copy, and not a wholde new plugin
							copyPlugin(monitor);
							return;
						} 
